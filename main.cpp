    ...
    vector<Airport*> getSuccessors(Airport airport) {
        ...
        for (auto route : routes) {
            ...
            if (rout->getSourceAirportID().compare(airport.getAirportID()) == 0) {
                ...
                Airport *desAirport = this->airports.at(des_airport_id);
                if (!(desAirport == NULL))
    ...
    bool isDestination(Airport airport) {
        ...
        for (auto temp : airports) {
            ...
            if (air->getAirportCity().compare(this->destinationCity) == 0 && air->getAirportCountry().compare(this->destinationCountry) == 0) {
                cout <<"Destination: "<< air->getAirportName() << endl;
                for (auto route : routes) {
                    Route *rout = route.second;
                    if (rout->getSourceAirportID().compare(airport.getAirportID()) == 0 &&
...
// void writeFinalOutputToFile(string filepath) {
//     FindRoute search;
//     vector<Airport> visitedAiports = search.breadthFirstSearch();
//     ofstream writer;
//     writer.open(filepath);
//     vector<Airport> revList;
//     for (int i = visitedAiports.size() - 1; i >= 0; i--) {
//         revList.push_back(visitedAiports.at(i));
//     }
//     revList.push_back(search.destinationAirport(search.dCity(), search.dCountry()));

//     int i = 0;
//     while (i < revList.size() - 1) {
//         writer << revList.at(i).getAirportICAO() << " from " <<  revList.at(i).getAirportIATA() << " to " << revList.at(i + 1).getAirportIATA()
//                        << " 0 stops\n";
//         i++;
//     }
//     writer << "Total flights: " << visitedAiports.size() << endl;
//     writer << "Total additional stops: " << "0";
//     writer.close();
// }
...
int main() {
    ...
    cout << visitedAiports.size() << endl;

    for(auto x : visitedAiports) {
        cout << x.getAirportID()<< " " << x.getAirportName() << " " << x.getAirportCity() << " " << x.getAirportCountry() << " " << x.getAirportIATA() << endl;
    }
    return 0;
}